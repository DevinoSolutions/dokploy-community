...
export const applicationRouter = createTRPCRouter({
	...
	deployNginxQuickstart: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			// The fork's `generateTraefikMeDomain` takes the project id (so the
			// organization/project wildcard base applies) and returns
			// `{ domain, baseDomain, source }` rather than a bare host string.
