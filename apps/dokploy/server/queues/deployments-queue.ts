...
let queueState = g[QUEUE_STATE_KEY];
if (!queueState) {
	queueState = {
...
const createWorker = (
	...
	const worker = new Worker<DeploymentJob>(
		...
		async (job) => {
			...
			const key = job.id
				? getInflightKey(targetKey, job.id)
				: `${getQueueName(targetKey)}:${job.timestamp}`;
			inflight.set(key, ac);
			try {
...
const startCancelSubscriber = async (): Promise<void> => {
	...
	sub.on("message", (_chan, payload) => {
		try {
			const { jobId, targetKey = LOCAL_TARGET } = JSON.parse(payload) as {
				jobId?: string;
				targetKey?: string;
			};
			if (!jobId) return;
			const ac = inflight.get(getInflightKey(targetKey, jobId));
			if (ac) ac.abort();
		} catch (err) {
...
const realDeploymentWorker = {
	...
	cancelJob: async (
		{ jobId, targetKey }: TargetedJobRef,
		_reason?: string,
	): Promise<void> => {
		const ac = inflight.get(getInflightKey(targetKey, jobId));
		if (ac) ac.abort();
	...
	hasInflight: ({ jobId, targetKey }: TargetedJobRef): boolean =>
		inflight.has(getInflightKey(targetKey, jobId)),
};

const noopDeploymentWorker = {
	run: () => Promise.resolve(),
	close: (_reason?: string) => Promise.resolve(),
	cancelJob: (_job: TargetedJobRef, _reason?: string) => Promise.resolve(),
	...
	hasInflight: (_job: TargetedJobRef) => false,
};
