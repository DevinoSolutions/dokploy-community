...
type GlobalQueues = typeof globalThis & {
	[QUEUES_KEY]?: Map<string, Queue<DeploymentJob>>;
};
const globalQueues = globalThis as GlobalQueues;
let queues = globalQueues[QUEUES_KEY];
if (!queues) {
	queues = new Map<string, Queue<DeploymentJob>>();
	globalQueues[QUEUES_KEY] = queues;
}
...
export const deploymentQueueManager = {
	...
	cancel: async (
		...
		await deploymentWorker.cancelJob({ jobId, targetKey });
