...
const { handleGiteaApplicationPullRequestEvent, isGiteaPullRequestEvent } =
	await import("@/server/utils/gitea-preview");
...
describe("gitea preview deployments with real deliveries", () => {
	...
	it("creates a preview from the 'opened' delivery", async () => {
		...
		const result = await handleGiteaApplicationPullRequestEvent({ application, body });
	...
	it("redeploys the existing preview from the 'synchronized' delivery", async () => {
		...
		await handleGiteaApplicationPullRequestEvent({ application, body });
	...
	it("deploys on 'label_updated' but never creates a preview on 'label_cleared'", async () => {
		await handleGiteaApplicationPullRequestEvent({
			application,
