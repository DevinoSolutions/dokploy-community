...
// A file listing can be large; the default child-process stdout cap is only
// 1 MB, at which `exec` throws. A scan MUST NOT silently return a partial list
// — an under-count would be treated as "already in sync" and let the cutover
// move serverId onto a target that never received the missing files. So we give
// local scans a generous-but-bounded buffer, and any overflow beyond it throws
// (see the re-throws below) so executeTransfer records a failure and aborts.
const SCAN_MAX_BUFFER = 128 * 1024 * 1024; // 128 MB
...
export async function scanVolume(
	...
	try {
		const { stdout } = serverId
			...
			: await execAsync(command, { maxBuffer: SCAN_MAX_BUFFER });
		...
		// Never return a partial list as if the scan succeeded — that would let
		// the cutover proceed against an incomplete diff. Fail hard instead.
		throw new Error(
			`Failed to scan volume ${volumeName}: ${
...
export async function scanBindMount(
	...
	try {
		...
		// the cutover proceed against an incomplete diff. Fail hard instead.
		throw new Error(
			`Failed to scan bind mount ${hostPath}: ${
				error instanceof Error ? error.message : String(error)
			}`,
		);
	}
