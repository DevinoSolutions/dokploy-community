...
export async function syncVolumeFilesBatch(
	...
	const sourceCommand = `docker run --rm -v ${shellEscape(
		...
		`echo ${encodedFileList} | base64 -d > /tmp/files.txt && tar czf - -C /volume_data -T /tmp/files.txt`,
	)}`;
	const targetCommand = `docker run --rm -i -v ${shellEscape(
		`${targetVolume}:/volume_data`,
	)} alpine tar xzf - -C /volume_data`;
...
export async function syncBindFile(
	...
	if (!sourceServerId && targetServerId) {
		...
		try {
			...
			const rsyncCommand = `rsync -azs --times -e ${shellEscape(
				rsyncSshTransport,
	...
	// Local -> Local
	await execAsync(
		`rsync -azs --times ${shellEscape(sourceFullPath)} ${shellEscape(targetFullPath)}`,
	);
}
