...
/**
 * List the open pull requests of a Gitea/Forgejo repository, shaped like the
 * GitHub and GitLab listings so the manual "Build Pull Request" dialog can
 * offer them. The author handle is carried so the manual path runs the very
 * same collaborator check the webhook handler runs against the pull request
 * author.
 */
export const getGiteaPullRequests = async (input: {
	giteaId?: string;
	owner: string;
	...
	const allPullRequests: GiteaPullRequest[] = [];
	let page = 1;
	const limit = 50;

	while (true) {
		const pullRequests = await giteaApiRequest<GiteaPullRequest[]>(
			input.giteaId,
			`/repos/${encodeURIComponent(input.owner)}/${encodeURIComponent(
				input.repositoryName,
			)}/pulls?state=open&sort=recentupdate&page=${page}&limit=${limit}`,
		);
	...
	return allPullRequests.map((pullRequest) => ({
		id: pullRequest.id,
		...
		// Gitea serializes the handle as `login`; `username` is the legacy alias.
		authorUsername:
			pullRequest.user?.login ?? pullRequest.user?.username ?? null,
		// Gitea authorizes by handle, like GitHub does.
		authorId: null,
	}));
};
