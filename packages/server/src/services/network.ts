...
/**
 * Server-side counterpart to the filtering the network picker applies in the
 * UI: a service may only reference overlay networks that belong to the caller's
 * organization and live on the same server as the service itself. Without this
 * the `networkIds` array on a service update is taken at face value, so a
 * crafted request could attach a network owned by another organization.
 * Returns the de-duplicated list to persist.
 */
