...
/**
 * Git providers that can host preview deployment *comments*. Preview
 * deployments need to read pull requests and write a status comment back to
 * them, so a provider only lands here once both capabilities exist.
 *
 * GitLab previews are supported by this fork too, but they surface status
 * through the GitLab webhook handler's merge request notes rather than through
 * this layer, so `getPreviewCommentContext` returns `null` for them.
 */
...
interface PreviewCommentResource {
	sourceType: string;
 ...
 * backs a preview deployment. Returns `null` for source types whose previews do
 * not write comments through this layer (gitlab, docker, custom git, ...).
 */
export const getPreviewCommentContext = (
	resource: PreviewCommentResource,
